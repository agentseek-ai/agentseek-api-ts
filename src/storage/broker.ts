...
export class RunBroker {
  ...
  // Appends an event and returns its seq (monotonic per run, starts at 0 for
  // each process incarnation — a requeued run republishes from scratch).
  append(runId: string, event: string, data: string | null, normalized: boolean | null): number {
    let buf = this.buffers.get(runId)
    if (!buf) {
      buf = { events: [], nextSeq: 0, finishedAt: null }
  ...
  // Starts the replay-window countdown; the buffer is swept TTL after finish.
  markFinished(runId: string): void {
    const buf = this.buffers.get(runId)
    if (buf && buf.finishedAt == null) buf.finishedAt = Date.now()
  }

  // Drops buffers of runs finished more than ttlMs ago. Buffers of live runs
  // are never touched.
  sweep(ttlMs: number): number {
    const now = Date.now()
    let removed = 0
    for (const [runId, buf] of this.buffers) {
      if (buf.finishedAt != null && now - buf.finishedAt > ttlMs) {
